...
from config.prompts import DEVOPS_SYSTEM_PROMPT
...
def devops_node(state: AgentState) -> AgentState:
    """DevOps agent: Docker, CI/CD, env, Linux — tools filtered by RBAC."""
    allowed = set(state.get("allowed_tools") or [])
    tools = [t for t in ALL_TOOLS if t.name in allowed]

    if not tools:
        msg = AIMessage(content="Permission denied: your role does not have access to any devops tools.")
        return {**state, "messages": [msg], "current_agent": "devops", "status": "done"}
    ...
    agent = create_react_agent(
        ...
        tools=tools,
        prompt=build_prompt_with_tools(DEVOPS_SYSTEM_PROMPT, list(allowed)),
    )
